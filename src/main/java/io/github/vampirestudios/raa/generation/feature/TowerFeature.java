            ...
            for (int j = -3; j <= 3; j++) {
                for (int k = -3; k <= 3; k++) {
                    ...
                    if (j == -2 && k == -2 && i == height) {
                        ...
                        LootableContainerBlockEntity.setLootTable(world, Rands.getRandom(), pos.add(j, i, k), (tier >= 1) ? (tier >= 2) ? LootTables.END_CITY_TREASURE_CHEST : LootTables.SIMPLE_DUNGEON_CHEST : RAALootTables.TOWER_LOOT);
                    }
                    if (tier >= 1) {
                        if (j == 2 && k == 2 && i == height) {

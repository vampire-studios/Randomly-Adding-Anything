...
public abstract class AbstractBC {
     ...
     */
    public abstract void generateColumn(int chunkX, int chunkZ, Chunk chunkIn, int localX, int localZ, int bottomY,
     ...
     */
    protected void digBlock(Chunk chunkIn, BlockState lavaBlock, int chunkX, int chunkZ, int localX, int localZ, int realY) {
        if (!lavaBlock.getFluidState().matches(FluidTags.WATER)) {
            // Check for adjacent water blocks to avoid breaking into lakes or oceans
            if (chunkIn.getBlockState(new BlockPos(localX, realY + 1, localZ)).getFluidState().matches(FluidTags.WATER)
                    || chunkIn.getBlockState(new BlockPos(localX, realY + 1, localZ)) == Blocks.WATER.getDefaultState())
                return;

            if (localX < 15 && (chunkIn.getBlockState(new BlockPos(localX + 1, realY, localZ)).getFluidState().matches(FluidTags.WATER))
                    || chunkIn.getBlockState(new BlockPos(localX + 1, realY, localZ)) == Blocks.WATER.getDefaultState())
                return;

            if (localX > 0 && (chunkIn.getBlockState(new BlockPos(localX - 1, realY, localZ)).getFluidState().matches(FluidTags.WATER))
                    || chunkIn.getBlockState(new BlockPos(localX - 1, realY, localZ)) == Blocks.WATER.getDefaultState())
                return;

            if (localZ < 15 && (chunkIn.getBlockState(new BlockPos(localX, realY, localZ + 1)).getFluidState().matches(FluidTags.WATER))
                    || chunkIn.getBlockState(new BlockPos(localX, realY, localZ + 1)) == Blocks.WATER.getDefaultState())
                return;

            if (localZ > 0 && (chunkIn.getBlockState(new BlockPos(localX, realY, localZ - 1)).getFluidState().matches(FluidTags.WATER))
                    || chunkIn.getBlockState(new BlockPos(localX, realY, localZ - 1)) == Blocks.WATER.getDefaultState())
                return;
     ...
     */
    protected void visualizeDigBlock(boolean digBlock, BlockState blockState, Chunk chunkIn, int localX, int realY, int localZ) {

...
public class FancyCliffsSurfaceBuilder extends SurfaceBuilder<CustomTernarySurfaceConfig> {
    ...
    public FancyCliffsSurfaceBuilder(Function<Dynamic<?>, ? extends CustomTernarySurfaceConfig> dynamicFunction) {
        super(dynamicFunction);
    }

    @Override
    public void generate(Random rand, Chunk chunk, Biome biome, int x, int z, int height, double noise, BlockState stone, BlockState water, int seaLevel, long seed, CustomTernarySurfaceConfig config) {
        x &= 15;
        z &= 15;
        height -= 1;
        if (noise > 0.5D && height > seaLevel + 1 && height < seaLevel + 12) {
            ...
            SurfaceBuilder.DEFAULT.generate(rand, chunk, biome, x, z, height, noise, stone, water, seaLevel, seed, config1);
        }
    }

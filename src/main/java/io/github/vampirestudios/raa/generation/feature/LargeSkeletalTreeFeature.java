...
public class LargeSkeletalTreeFeature extends AbstractTreeFeature {
    ...
    private static boolean isSurroundedByAir(TestableWorld testableWorld, BlockPos blockPos, Direction ignore) {
        for (Direction direction : Direction.Type.HORIZONTAL) {
            ...
            if (!isAirOrLeaves(testableWorld, blockPos.offset(direction))) return false;
        }
        return true;
    }
    ...
    public boolean generate(ModifiableTestableWorld modifiableTestableWorld, Random random, BlockPos blockPos, Set<BlockPos> set, Set<BlockPos> set1, BlockBox blockBox, TreeFeatureConfig treeFeatureConfig) {
        if (!isDirtOrGrass(modifiableTestableWorld, blockPos.down(1))) return false;
    ...
    private void generateBranch(ModifiableTestableWorld world, Random random, BlockPos pos, BlockBox blockBox, int maxBranchouts, Direction lastDir) {
        ...
        for (int int_4 = 0; int_4 < baseHeight; ++int_4) {
            ...
            addLog(world, blockPos, Direction.Axis.Y);
        }
        ...
        if (maxBranchouts > 0) {
            for (int int_6 = 0; int_6 < int_5; ++int_6) {
                ...
                if (Math.abs(blockPos_4.getX() - pos.getX()) < int_1 && Math.abs(blockPos_4.getZ() - pos.getZ()) < int_1 && isAirOrLeaves(world, blockPos_4) && isAirOrLeaves(world, blockPos_4.down(1)) && isSurroundedByAir(world, blockPos_4, direction_1.getOpposite())) {
                    addLog(world, blockPos_4, direction_1.getAxis());

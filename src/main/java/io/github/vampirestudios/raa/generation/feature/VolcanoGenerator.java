...
import io.github.vampirestudios.raa.utils.SimpleRadialNoise;
...
public class VolcanoGenerator extends StructurePiece {
	...
	VolcanoGenerator(Random random, int centerX, int centerZ, Biome biome, DimensionData dimensionDataIn) {
		super(VolcanoGenerator::new, 0);
		this.dimensionData = dimensionDataIn;
	...
	public VolcanoGenerator(StructureManager manager, CompoundTag tag) {
		super(VolcanoGenerator::new, tag);
			...
			for (int x = boundingBox.minX; x <= boundingBox.maxX; x++) {
				...
				if (chamberShape > 0.0) {
					for (int dY = -chamberDY; dY <= chamberDY; dY++) {
						...
						iWorld.setBlockState(pos, Blocks.LAVA.getDefaultState(), 2);
						iWorld.getFluidTickScheduler().schedule(pos, iWorld.getFluidState(pos).getFluid(), 0);
					}
				} else if (chamberShape > -0.1) {
					pos.set(x, chamberMiddle, z);
					iWorld.setBlockState(pos, pickRandomChamberBlock(true, dX, dZ), 2);
				}
				...
				if (dZ == 0 && (dX >= -1 && dX <= 1) || dX == 0 && (dZ >= -1 && dZ <= 1)) {
					for (int dY = -(lavaTubeLength + chamberHeight); dY < lavaHeight; dY++) {
						...
						if (underwater && dY == lavaHeight - 1) {
							...
							iWorld.setBlockState(pos, state, 2);
						} else {
							iWorld.setBlockState(pos, Blocks.LAVA.getDefaultState(), 2);
							iWorld.getFluidTickScheduler().schedule(pos, iWorld.getFluidState(pos).getFluid(), 0);
						}
					...
					iWorld.setBlockState(pos.set(x, chamberMiddle + chamberDY + 1, z), pickRandomChamberBlock(true, dX, dZ), 2);
					iWorld.setBlockState(pos.set(x, chamberMiddle - chamberDY - 1, z), pickRandomChamberBlock(false, dX, dZ), 2);
				}
				...
				BlockState top = Registry.BLOCK.get(new Identifier(RandomlyAddingAnything.MOD_ID, dimensionData.getName().toLowerCase() + "_stone")).getDefaultState();
				...
				int startY = iWorld.getTopY(Heightmap.Type.OCEAN_FLOOR_WG, x, z) - baseY;

				for (int dY = startY; dY < columnHeight - 1; dY++) {
					pos.set(x, baseY + dY, z);

					if (iWorld.getBlockState(pos).isAir() || iWorld.getFluidState(pos).getFluid() == Fluids.WATER) {
						iWorld.setBlockState(pos, Registry.BLOCK.get(new Identifier(RandomlyAddingAnything.MOD_ID, dimensionData.getName().toLowerCase() + "_stone")).getDefaultState(), 2);
					}
				}
				...
				if (baseY < 60 || !iWorld.getBlockState(pos).isAir()) {
					if (underwater && random.nextInt(80) == 0) {
						top = Blocks.MAGMA_BLOCK.getDefaultState();
					} else {
						top = Registry.BLOCK.get(new Identifier(RandomlyAddingAnything.MOD_ID, dimensionData.getName().toLowerCase() + "_stone")).getDefaultState();
					}
				...
				if ((iWorld.getBlockState(pos).isAir() || iWorld.getFluidState(pos).getFluid() == Fluids.WATER) && startY < columnHeight) {
					iWorld.setBlockState(pos, top, 2);
	...
	private BlockState pickRandomChamberBlock(boolean top, int dX, int dZ) {
		...
		if (diamondNoise > 0.70) {
			...
			return obsidianNoise > 0.25 ? Blocks.OBSIDIAN.getDefaultState() : Registry.BLOCK.get(new Identifier(RandomlyAddingAnything.MOD_ID, dimensionData.getName().toLowerCase() + "_stone")).getDefaultState();
		}
	}
}
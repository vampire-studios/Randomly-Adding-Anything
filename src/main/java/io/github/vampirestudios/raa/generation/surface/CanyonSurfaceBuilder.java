...
public class CanyonSurfaceBuilder extends SurfaceBuilder<TernarySurfaceConfig> {

    private static final OpenSimplexNoise CLIFF_NOISE = new OpenSimplexNoise(346987);

    public CanyonSurfaceBuilder(Function<Dynamic<?>, ? extends TernarySurfaceConfig> function) {
        super(function);
    }
    ...
    @Override
    public void generate(Random rand, Chunk chunk, Biome biome, int x, int z, int vHeight, double noise, BlockState stone, BlockState water, int var11, long seed, TernarySurfaceConfig config) {
        if (vHeight < 62 + 5) {
            ...
            SurfaceBuilder.DEFAULT.generate(rand, chunk, biome, x, z, vHeight, noise, stone, water, var11, seed, config);

            return;
        }

        BlockPos.Mutable pos = new BlockPos.Mutable(x, 62 - 1, z);
        ...
        if (vHeight < 62 + 8 && cliffNoise > 0.3) {
            // seaLevel+5 -> 1/4, seaLevel+6 -> 2/4, seaLevel+7 -> 3/4

            cliffNoise *= (vHeight - 62 - 3) * 0.25;
        }
